...
if __name__ == '__main__':
    ...
    parser.add_argument("--pre_query", action='store_true', default=False)
        ...
        for i in tqdm(range(len(reasoning_prompts))):
                ...
                with torch.no_grad():
                    ...
                    if args.match_type == 'clip':
                        ...
                        if os.path.exists(clip_embedding_path):
                            ...
                            qwen_box_save_path = os.path.join(save_path, cam.image_name, f'{reasoning_prompt}_qwen_box.pt')
                            qwen_mask_save_path = os.path.join(save_path, cam.image_name, f'{reasoning_prompt}_qwen_mask.pt')
                            ...
                            if args.match_type == 'clip':
                                ...
                                torch.save(box, qwen_box_save_path)
                                torch.save(mask, qwen_box_save_path)
                        ...
                        instance_indexes.append(most_relevant_instance_index)
                    else:
                        qwen_box_save_path = os.path.join(save_path, cam.image_name, f'{reasoning_prompt}_qwen_box.pt')
                        qwen_mask_save_path = os.path.join(save_path, cam.image_name, f'{reasoning_prompt}_qwen_mask.pt')
                        ...
                        if args.match_type == 'box': 
                            if os.path.exists(qwen_box_save_path):
                                box = torch.load(qwen_box_save_path)
                            else:
                                _, box, image_pil = reasoning_grouding_by_qwen(image_path, qwen_template1(reasoning_prompt))
                                torch.save(box, qwen_box_save_path)
                            instance_indexes.append(get_instance_embeddings_by_box(gaussian, box, instance_feature))
                        else:
                            if os.path.exists(qwen_mask_save_path):
                                mask = torch.load(qwen_mask_save_path)
                            else:
                                _, box, image_pil = reasoning_grouding_by_qwen(image_path, qwen_template1(reasoning_prompt))
                                torch.save(box, qwen_box_save_path)
                                image_np = np.array(image_pil)
                                ...
                                mask = torch.from_numpy(masks[0])
                                torch.save(mask, qwen_box_save_path)
                            instance_indexes.append(get_instance_embeddings_by_mask(gaussian, mask, instance_feature))
            ...
            for j in tqdm(range(len(colmap_eval_cameras))):
                ...
                with torch.no_grad():
                    ...
                    Image.fromarray(mask_map).save(os.path.join(mask_map_save_path, f'{reasoning_prompt}_{args.clip_type}_{args.train_views}.png'))
                    Image.fromarray(pred_mask).save(os.path.join(mask_save_path, f'{reasoning_prompt}_{args.clip_type}_{args.train_views}.png'))
    ...
    if args.pre_query:
        save_name = f'gsgrouping_{args.match_type}_{args.train_views}_pre_query.txt'
    else:
