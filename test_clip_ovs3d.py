...
if __name__ == '__main__':
    ...
    for scene_name in tqdm(scene_names):
        ...
        print(clip_text_prompts)
        clip.set_positives(clip_text_prompts)
        valid_index = 0
        for i in tqdm(range(len(colmap_cameras))):
            ...
            valid_index += 1
            with torch.no_grad():
                mask_map_save_path = os.path.join(args.clip_save_path, cam.image_name, 'mask_maps')
                mask_save_path = os.path.join(args.clip_save_path, cam.image_name, 'masks')
                mask_object_save_path = os.path.join(args.clip_save_path, cam.image_name, 'objects')
                ...
                Image.fromarray((apply_colormap(render_feature.permute(1, 2, 0), ColormapOptions(colormap="pca", pca_dict=rendered_feature_pca_dict)).cpu().numpy() * 255).astype(np.uint8)).save(os.path.join(args.clip_save_path, cam.image_name, f'rendered_feature_pca.jpg'))
                Image.fromarray((apply_colormap(instance_feature.permute(1, 2, 0), ColormapOptions(colormap="pca", pca_dict=instance_feature_pca_dict)).cpu().numpy() * 255).astype(np.uint8)).save(os.path.join(args.clip_save_path, cam.image_name, f'instance_feature_pca.jpg'))
                if instance_embeddings == []:
                    for j in range(len(clip.positives)):
                        clip_embeddings = []
                        for value in gaussian.clip_embeddings.values():
                            clip_embeddings.append(value)
                        clip_embeddings = torch.stack(clip_embeddings)
                        N, I, D = clip_embeddings.shape
                        clip_embeddings = clip_embeddings.reshape(N*I, D)
                        relevancy = clip.get_relevancy(clip_embeddings, j)[..., 0]
                        mask_index = relevancy.argmax()
                        mask_index = mask_index % I
                        print(relevancy.max(), mask_index)
    ...
    with open(f'clip_ovs3d.txt', 'w') as f:
        f.write(metrics)
